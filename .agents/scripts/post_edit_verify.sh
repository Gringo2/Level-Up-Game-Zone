...
# Phase 3: Micro-Verification + TDD Order Check + AST Blast Radius Engine (ADR-007)
...
if [ -f "$FAIL_FILE" ]; then
  ...
  if [ "$count" -ge 3 ]; then
    if [ ! -f "$RCA_FILE" ]; then
      echo "🚫 EXHAUSTION PROTOCOL ACTIVE: This file has failed verification 3 times." >&2
      echo "   You must create an RCA document at .agents/.scratch/RCA.md before you can edit it again." >&2
      exit 1
    else
      echo "✅ RCA Document found. Exhaustion lock lifted for this attempt." >&2
...
function exhaustion_trap {
  ...
  if [ $EXIT_CODE -ne 0 ]; then
    ...
    if [ "$count" -ge 3 ]; then
      echo "🚨 EXHAUSTION TRIGGERED: Verification failed 3 times. RCA document now required." >&2
    fi
  else
    rm -f "$FAIL_FILE"
  fi
...

{
  ...
  if [ ! -f "$TARGET_FILE" ]; then
    ...
    if [ $DEPCRUISE_EXIT -ne 0 ] || [ -z "$GRAPH_JSON" ]; then
      ...
      # --- 9. Boundary Enforcement: ESLint / Biome on affected packages ---
