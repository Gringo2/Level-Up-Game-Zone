...
*   [x] **Mission 111:** Reports & Expenses Responsive Layout Hardening — eliminate horizontal container overflow on /reports (320px, 768px) and /expenses (768px) via lg:flex-row header breakpoint harmonization, min-[360px]:flex-row date picker stacking, and history card header flex-wrapping (ACP-019).
*   [x] **Mission 112:** Cross-Flow State Synchronization & Range Guard Hardening — Dashboard Float Update in-flight guard, Expenses filter-aware history mutations, Reports & SalaryReport date range inversion guards, and Credits employee filter containment (ACP-020).
