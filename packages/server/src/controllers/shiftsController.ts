...
export const closeShift = async (req: Request, res: Response) => {
  try {
    ...
    const [gameSalesSnap, kenoSnap, creditsSnap, expensesSnap] = await Promise.all([
    ...
    const totalGameSales = gameSalesSnap.docs.reduce((sum: number, doc: any) => sum + (doc.data().calculated_total || 0), 0);
    const totalKenoNet = kenoSnap.docs.reduce((sum: number, doc: any) => sum + (doc.data().net_profit || 0), 0);
    const pendingCredits = creditsSnap.docs.filter((doc: any) => doc.data().status === 'Pending').reduce((sum: number, doc: any) => sum + (doc.data().amount || 0), 0);
    const totalExpenses = expensesSnap.docs.reduce((sum: number, doc: any) => sum + (doc.data().amount || 0), 0);

    const expectedCash = (shiftData.opening_float || 0) + totalKenoNet + totalGameSales - totalExpenses - pendingCredits;
    const variance = Number(actualCashCounted) - expectedCash;

    if (Math.abs(variance) > 2 && !shortageReason) {
      return res.status(400).json({ error: 'Variance is greater than $2.00. Please provide a reason for the shortage.' });
    }
    ...
    return res.status(200).json({ message: 'Shift closed successfully', data: updateData });
  } catch (error: any) {
